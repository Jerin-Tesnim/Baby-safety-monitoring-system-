...
def load_compatible_lstm_model(model_path):
    # Load a Keras 3 model in an environment that uses Keras 2.x.
    # Keras 3 stores InputLayer shape as "batch_shape", while Keras 2.x
    # expects the same value under "batch_input_shape".
    source_path = Path(model_path)
    original_error_text = "Unknown model loading error"
    ...
    try:
        ...
        print("Normal LSTM loading failed. Applying compatibility fix...")
    ...
    def clean_config(value):
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                # Convert the Keras 3 InputLayer key to the Keras 2.x key.
                if key == "batch_shape":
                    cleaned["batch_input_shape"] = clean_config(item)
                else:
                    cleaned[key] = clean_config(item)
    ...
    if not zipfile.is_zipfile(source_path):
        raise RuntimeError(
            ...
            f"Original loading error: {original_error_text}"
        )

    fixed_path = (
        source_path.parent
        / "lstm_behavior_model_compatible.keras"
    )

    temporary_folder = Path(
        tempfile.mkdtemp(prefix="lstm_compatibility_")
    )

    try:
        with zipfile.ZipFile(source_path, "r") as archive:
            archive.extractall(temporary_folder)

        config_path = temporary_folder / "config.json"

        if not config_path.exists():
            raise RuntimeError(
                "config.json was not found inside the LSTM .keras model."
            )

        with config_path.open("r", encoding="utf-8") as file:
            config = json.load(file)

        fixed_config = clean_config(config)

        with config_path.open("w", encoding="utf-8") as file:
            json.dump(fixed_config, file)
        ...
        with zipfile.ZipFile(
            ...
            for file_path in temporary_folder.rglob("*"):
                if file_path.is_file():
                    archive.write(
                        file_path,
                        file_path.relative_to(temporary_folder)
                    )
        ...
        print("Compatible LSTM model loaded successfully.")
        print("Compatible model saved at:", fixed_path)
        return model

    except Exception as compatibility_error:
        raise RuntimeError(
            "LSTM compatibility conversion failed. "
            f"Original error: {original_error_text}. "
            f"Compatibility error: {compatibility_error}"
